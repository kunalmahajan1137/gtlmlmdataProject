...
$sqluser = "SELECT user_id FROM users WHERE id = ?";
$stmt1 = $con->prepare($sqluser);
...
$sql = "SELECT * FROM IncomeDetails WHERE user_id = ?";
...
<div class="header">
  <div class="row d-flex flex-wrap align-items-center justify-content-between">
    <div class="col-md-6 col-12 d-flex justify-content-start">
      <!--<span class="title text-truncate">My Referral Link</span>-->
    </div>
    <div class="col-md-6 col-12 d-flex justify-content-md-end justify-content-start overflow-hidden">
      <div class="breadcrumb text-nowrap text-truncate ">
        <!--<a href="./account.php" class="breadcrumb-link text-capitalize ">ACCOUNT&nbsp;</a> / <span class="text-uppercase">&nbsp;My Referral Link</span>-->
      </div>
    </div>
  </div>
</div>
...
if ($result1->num_rows > 0) {
    while ($row1 = $result1->fetch_assoc()) { // Corrected variable usage
        $user_id = $row1['user_id'];
        $name = isset($row1['name']) ? $row1['name'] : "Unknown"; // Handle missing name
        $referralLink = "https://thecpsolution.com/mainregister.php?sponsor_id=" . $user_id;
        ...
        // echo "<td>" . $name . "</td>";
        echo "<td>";
        echo '<input type="text" class="form-control" value="' . $referralLink . '" readonly>';
        echo "</td>";
        echo "<td>";
        echo '<a href="https://wa.me/?text=' . urlencode("Register using this link: " . $referralLink) . '" target="_blank" class="btn btn-success btn-sm me-2">';
        echo '<i class="fab fa-whatsapp"></i> Share</a>';
        echo '<a href="' . $referralLink . '" target="_blank" class="btn btn-primary btn-sm">';
        echo '<i class="fas fa-user-plus"></i> Register</a>';
        echo "</td>";
        echo "</tr>";
    }
} else {
